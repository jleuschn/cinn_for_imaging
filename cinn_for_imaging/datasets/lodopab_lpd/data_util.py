...
from cinn_for_imaging.datasets.lodopab_lpd.learnedpd_dataset import LoDoPaBLearnedPDDataset
from dival.datasets.cached_dataset import generate_cache_files, CachedDataset



class LoDoPaBLPDDataModule(pl.LightningDataModule):

    def __init__(self, batch_size:int = 4, num_data_loader_workers:int = 0, impl:str = 'astra_cuda'):
        """
    ...
    def setup(self, stage:str = None):
        ...
        dataset = LoDoPaBLearnedPDDataset(impl=self.impl)
        self.ray_trafo = dataset.dataset.get_ray_trafo(impl=self.impl)
        ...
        if stage == 'fit' or stage is None:
            self.lodopab_train = cached_dataset.create_torch_dataset(part='train',
                                    reshape=((1,) + dataset.space[0].shape,
                                    (1,) + dataset.space[1].shape))
            
            self.lodopab_val = cached_dataset.create_torch_dataset(part='validation',
                                    reshape=((1,) + dataset.space[0].shape,
                                        (1,) + dataset.space[1].shape))
