...
dataset = LoDoPaBDataModule(batch_size=1, impl='astra_cuda', num_data_loader_workers=0)
...
version = 'version_0' 
chkp_name = 'epoch=0'
path_parts = ['..', 'experiments', 'invGD', 'default',
...
num_images = 5
with torch.no_grad():
    for i, batch in tqdm(zip(range(num_images), dataset.test_dataloader()),
                         total=num_images):
        obs, gt = batch
        obs = obs.to('cuda')
        gt = gt.numpy()[0][0]
        ...
        z = torch.zeros((1,reconstructor.img_size[0]*reconstructor.img_size[1]),
                                        device=reconstructor.model.device)

        xgen, _ = reconstructor.model(cinn_input=z, cond_input=obs,
                                    rev=True)
        _, ax = plot_images([xgen[0,0,:,:].cpu(), gt],
...
with torch.no_grad():
    for i, batch in tqdm(zip(range(num_test_images),dataset.test_dataloader()), 
        ...
        if plot_examples:
            if i < 3: # plot results for the first 3 images + reconstruction
                gt = gt.cpu().numpy()[0][0]
        ...
        for i, batch in tqdm(zip(range(3), dataset.test_dataloader()),
            ...
            if check_inv_gt:
                ...
                reco_gt, _ = reconstructor.model(cinn_input=z,
                                              cond_input=obs,
                                              rev=True)
                recos_gt.append(reco_gt.detach().cpu().numpy()[0][0])
                inv_psnr_gt.append(PSNR(reco_gt.detach().cpu().numpy()[0][0],
...
with torch.no_grad():
    for i, batch in tqdm(zip(range(1), dataset.test_dataloader()),
        ...
        z = torch.randn((num_images,reconstructor.img_size[0]*reconstructor.img_size[1]),
                                        device=reconstructor.model.device)
        obs_rep = torch.repeat_interleave(obs,num_images,dim = 0)

        xgen, _ = reconstructor.model(cinn_input=z, cond_input=obs_rep,
                                    rev=True)
