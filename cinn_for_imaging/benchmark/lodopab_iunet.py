...
num_test_images = 5#len(dataset.test_dataloader())
...
with torch.no_grad():
    for i, batch in tqdm(zip(range(num_test_images),dataset.test_dataloader()), 
        ...
        if psnrs[-1] < 20:
            ...
            ax2.set_title("SSIM: " + str(ssims[-1]) + " PSNR: " + str(psnrs[-1]))
            plt.savefig("img_{}.pdf".format(i))
            plt.show()
...
if plot_examples:
    for i, batch in tqdm(zip(range(3), dataset.test_dataloader()),
        ...
        plt.savefig('test sample iunet {:d}'.format(i)+'.pdf')
        if save_report:
